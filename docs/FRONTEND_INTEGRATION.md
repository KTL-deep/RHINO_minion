...
Поле prompt вызывает `POST /api/sessions/{session_id}/prompt`. План строится backend-компонентом, выполняется одним Rhino batch и возвращается в интерфейс. Последовательность инструментов последнего плана отображается под prompt.
