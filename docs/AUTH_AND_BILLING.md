...
Desktop-приложение остаётся чистым чат-клиентом. Оно проверяет entitlement при запуске и периодически во время работы. Если активной подписки или доступных кредитов нет, клиент открывает отдельный сайт RHINO Minion для входа, покупки и управления аккаунтом; checkout и тарифные экраны внутрь desktop-приложения не встраиваются.
